...
func TestBackup2B(t *testing.T) {
	...
	cfg.disconnect((leader1 + 1) % servers)
	//log.Printf("after second disconnect")
	...
	for iters := 0; iters < 1000; iters++ {
		...
		for i := 0; i < servers; i++ {
			...
			if ok && cfg.connected[i] {
				DPrintf("goroutine[%d] call server[%d]'s start()", os.Getgid(), i)
				leader = i
			}
		}
		//log.Printf("1111")
		...
		if leader != -1 && (rand.Int()%1000) < int(RaftElectionTimeout/time.Millisecond)/2 {
			...
			//log.Printf("leader[%d] disconnect", leader)
			nup -= 1
		}
		//log.Printf("3333333")
		if nup < 3 {
